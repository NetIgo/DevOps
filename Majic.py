...
def main(args):
    ...
    # Fitre les devices qui ont pas d'ip primaire
    ...
    for device in itertools.chain(devices, vm, ips):
        if device.custom_fields.get(args.custom_field):
            ...
            try:
                ...
                continue  # Ignore les erreurs pendants le loading en json des targets 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', default=10000,
                        help='Default target port')
    parser.add_argument('-f', '--custom-field', default='prom_labels',
                        help='Netbox custom field to use to get the target labels')
    parser.add_argument('url', help='URL de Netbox')
    parser.add_argument('token', help='Authentication Token')
    parser.add_argument('output', help='Output file ou le fichier sd va apparaitre')
